...
async def aliehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("اضغط لاستخراج الجلسه", url=f"https://t.me/FreedomLoBoT"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/SOURCE_MARVEN"),
            ],
        ]
    )
...
async def alivehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝙶𝚁𝙾𝚄𝙿️", url=f"https://t.me/D3M_MARVEN"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/SOURCE_MARVEN"),
            ],
        ]
    )
...
async def caesar(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("《 ⎖᳒𝘿𝙀𝙑 𝘿𝙀𝙑𝘼𝙄𝙇》", url=f"https://t.me/Zo_Mbi_e"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/SOURCE_MARVEN"),
            ],
        ]
    )
...
async def makedzombie(client, message):
  ...
  if message.from_user.username in DEVS:
    ...
    try:
      ...
      return await message.reply_text("قم بارسال الايدي بشكل صحيح")#'‹ ٰ💸 ⇣ سورس مارڤين ⇣ 💸 › .'#
